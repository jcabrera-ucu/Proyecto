...
namespace Library
{
    ...
    public abstract class BaseHandler : IHandler
    {
        ...
        public IHandler? Handle(Message message, out string response1, out string response2)
        {
            if (this.InternalHandle(message, out response1, out response2))
            {
                return this;
            }
            else if (this.Next != null)
            {
                return this.Next.Handle(message, out response1, out response2);
            }
            else
            {
                return null;
